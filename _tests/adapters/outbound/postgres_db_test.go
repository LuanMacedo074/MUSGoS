...
// The two tests below need no server: they verify NewPostgresDB rejects a bad
// connection string, so `make test` keeps some coverage of the adapter's
// construction path even without TEST_POSTGRES_DSN.

func TestNewPostgresDB_EmptyDSN(t *testing.T) {
	if _, err := outbound.NewPostgresDB(""); err == nil {
		t.Error("expected error for empty connection string")
	}
	if _, err := outbound.NewPostgresDB("   "); err == nil {
		t.Error("expected error for blank connection string")
	}
}

func TestNewPostgresDB_ConnectionFailure(t *testing.T) {
	// Valid DSN syntax, but nothing is listening on port 1 — the Ping in
	// NewPostgresDB must surface a connection error. connect_timeout guards
	// against a hang if the refusal is slow.
	_, err := outbound.NewPostgresDB("postgres://user:pass@127.0.0.1:1/db?sslmode=disable&connect_timeout=2")
	if err == nil {
		t.Error("expected connection error when no postgres is listening")
	}
}
