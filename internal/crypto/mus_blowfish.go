...
//O projeto original OpenSMUS está licenciado sob a Licença MIT, sendo compatível tanto com desenvolvimento open source (GPL ou não) quanto comercial.
//Copyright original: © 2001-2008 Mauricio Piacentini
//Utilizei o projeto dele como base e referência para desenvolver esta versão, além de ter entrado e contato e recebido os arquivos originais em C.
...
func (b *Blowfish) SetKey() {
	key := b.key
	keyLen := len(key)
	if keyLen > (BlowfishRounds+2)*4 {
		keyLen = (BlowfishRounds + 2) * 4
		key = key[:keyLen]
	}
	...
	for i := 0; i < BlowfishRounds+2; i++ {
		...
		ri := uint32(b0)<<24 | uint32(b1)<<16 | uint32(b2)<<8 | uint32(b3)
...
func (b *Blowfish) DoCrypto(data []uint32, encrypt int) {
	p := b.initBoxes.P[:]
	s := b.initBoxes.S[:]
	l := data[0]
	r := data[1]
...
func (b *Blowfish) mungeData(inBuffer []byte, operation int) []byte {
	curStateOffset := b.stateOffset
	l := len(inBuffer)
	d := make([]byte, 8)
	ti := []uint32{0, 0}
	save := false
	...
	binary.BigEndian.PutUint32(stateBytes[0:4], b.state)
	binary.BigEndian.PutUint32(stateBytes[4:8], 0)

	v0 := binary.BigEndian.Uint32(stateBytes[0:4])
	v1 := binary.BigEndian.Uint32(stateBytes[4:8])

	ti[0] = v0
	ti[1] = v1

	binary.BigEndian.PutUint32(d[0:4], v0)
	binary.BigEndian.PutUint32(d[4:8], v1)

	outBuffer := make([]byte, l)

	for i := 0; i < l; i++ {
		if curStateOffset == 0 {
			b.DoCrypto(ti, kEncrypt)

			binary.BigEndian.PutUint32(d[0:4], ti[0])
			binary.BigEndian.PutUint32(d[4:8], ti[1])

			save = true
		}

		outBuffer[i] = inBuffer[i] ^ d[curStateOffset]

		curStateOffset = (curStateOffset + 1) & 0x07
	}
	...
	if save {
		...
		binary.BigEndian.PutUint32(stateBytes[0:4], v0)
		binary.BigEndian.PutUint32(stateBytes[4:8], v1)

		b.state = v0
	}

	b.stateOffset = curStateOffset
